...
async function handle_cmds(message) {
  ...
  if (words[0] == cmdprefix + "send" || words[0] == cmdprefix + "dm"){
    ...
    //
  } else if (words[0] == cmdprefix + "viewc" || words[0] == cmdprefix + "view_channel"){
    let channelid = words[1]
    if (channelid) {
      channelid = toText(channelid).substring(0, 1024).replace(/\D/g, '');
    } else {
       mchannel.send(`channel ${toText(channelid).substring(0, 1024)} not found`)
      return;
    }
      ...
      prevmessages.forEach((msg) => { 
        if (msg.attachments.size > 0){
          msg.attachments.forEach(mattach => {
            desctxt = desctxt + "(attachment){" + mattach.url + ", " + mattach.name + "}\n";
          })
        }
    ...
    //
  } else if (message.content == cmdprefix + "info" || message.content == cmdprefix + "stats"){
    ...
    const newEmbed = new discordjs.EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Cpu', value: toText((process.cpuUsage().system / 1000000).toFixed(2)) + "%", inline: true },
        { name: 'Storage', value: toText(((os.totalmem() - os.freemem()) / 1073741824).toFixed(2)) + " / " + toText((os.totalmem() / 1073741824).toFixed(2)) + " GB" , inline: true },
        { name: 'Heap', value: toText((process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)) + " / " + toText((process.memoryUsage().heapTotal / 1024 / 1024).toFixed(2)) + " MB" , inline: true },
